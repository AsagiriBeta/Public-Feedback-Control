function varargout = pcd_version(cmd, varargin)
%PCD_VERSION 程序版本号的**唯一出处**。
%
%   v = pcd_version()            当前版本字符串，如 '0.1.0'
%   pcd_version('label')         带 v 前缀，如 'v0.1.0'（界面标题用）
%   tf = pcd_version('gt', a, b) a 比 b 新返回 true
%   tf = pcd_version('eq', a, b) 版本相同返回 true
%
...
switch lower(char(cmd))
    ...
    case 'eq'
        varargout{1} = vcmp(varargin{1}, varargin{2}) == 0;
    otherwise
        error('pcd:version:cmd', '未知命令 %s', cmd);
end
end
