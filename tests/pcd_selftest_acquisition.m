function pcd_selftest_acquisition()
%PCD_SELFTEST_ACQUISITION 采集链自检：不需要接仪器也能跑。
%
%   pcd_selftest_acquisition
%
...
% 这是回归护栏：改动 rigol/ 或 pcd_run_experiment 的采集部分后请跑一遍。
...
td = fullfile(tempdir, sprintf('pcd_save_selftest_%s', datestr(now, 'yyyymmdd_HHMMSS')));
fs = 40e6;
f0 = 1.5e6;
nn = 2048;
tt = (0:nn-1) / fs;
...
S.n_expect = 2;
S.npts = nn;
S.params = pcd_save_params('open_mb', struct('freq_mhz', 1.5, 'volt_mVpp', 20, ...
    'prf_hz', 1, 'n_cycle', 100, 'npts', nn, 'realFs', fs));
try
    rundir = pcd_save_acquisition(td, 'OpenMB_selftest', S);
    ...
    same = pcd_save_acquisition(td, 'OpenMB_selftest', S, struct('plots', false, 'rundir', rundir));
...
% ---------------------------------------------------------------- 5) 空化率 / 功放
fprintf('\n=== 5) 空化率 ↔ 周期数、功放增益 ===\n');
fails = fails + expect(abs(pcd_duty('cav_pct', 3000, 1.5, 5) - 1) < 1e-12, ...
    '1.5 MHz PRF 5 Hz 3000 cyc → 空化率 1%');
fails = fails + expect(pcd_duty('n_cycle', 1, 1.5, 5) == 3000, '空化率 1% → 3000 周期');
fails = fails + expect(pcd_duty('n_cycle', 1, 1.5, 2) == 7500, '空化率 1% @ PRF 2 Hz → 7500 周期');
fails = fails + expect(pcd_duty('n_cycle', 100, 1.5, 2) == 10000, ...
    '空化率过大时周期数截到 10000（不静默 3000）');
p9990 = pcd_fus_params(struct('freq_mhz', 1.5, 'volt_mVpp', 50, 'prf_hz', 2, ...
...
Psave = pcd_save_params('open_mb', struct('freq_mhz', 1.5, 'volt_mVpp', 100, ...
