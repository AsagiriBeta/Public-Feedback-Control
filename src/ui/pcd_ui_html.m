function ui = pcd_ui_html(fig)
%PCD_UI_HTML 界面的「UI 适配层」：把界面操作收敛成一组函数句柄交给算法层。
% 参数、状态、波形、点图都通过 uihtml 的 Data 通道推给前端（web/ 下的 Alpine 页面）。
% 约定接口见 pcd_ui_check；算法层只依赖这层，不依赖 uihtml 或任何具体控件。
ui = struct( ...
    'params',     @() pcd_fus_params(getappdata(fig, 'pcd_params')), ...
    'raw',        @() getappdata(fig, 'pcd_params'), ...
    'debugfv',    @() html_debugfv(fig), ...
    'outdir',     @() html_outdir(fig), ...
    'clearTrend', @() pcd_ui_push(fig, struct('cmd', 'clearTrend')), ...
    'status',     @(s) pcd_ui_push(fig, struct('cmd', 'status', 'text', char(string(s)))), ...
    'countdown',  @(remain, total) html_countdown(fig, remain, total), ...
    'waveform',   @(varargin) html_waveform(fig, varargin{:}), ...
    'trend',      @(k, sc, ic, v, xmax, ymax) pcd_ui_push(fig, struct('cmd', 'trend', ...
...
if do_fft
    [F, ~, db] = pcd_spectrum(y, fs);
    [f_pk, db_pk] = pcd_fft_peak_mhz(F, db, 0.3, 8);
end
