function fig = pcd_ui_app()
%PCD_UI_APP 新版界面宿主：uifigure + uihtml（前端 Alpine.js + uPlot）。
%
% 界面本体是 web/ 下的静态页面，通过 uihtml 嵌进 uifigure；MATLAB 只负责仪器与算法，
% 两者用 uihtml 的通道通信：
%   MATLAB -> JS : sendEventToHTMLSource(h,'pcd',S)  （启动瞬间可写一次 h.Data）
%   JS -> MATLAB : sendEventToMATLAB('action'|'params'|'ready', ...)  -> pcd_ui_event
% 相比 GUIDE：布局交给 CSS（自适应/DPI/圆角全不用手写），图表交给 uPlot。
root = pcd_web_root();
src = fullfile(root, 'index.html');
if ~isfile(src)
    error('PCD:ui:noWeb', '找不到前端资源：%s', src);
end
...
fig = uifigure('Name', ['PCD  ·  DHO814 / DG2052  ·  ' pcd_version('label')], ...
...
S = pcd_prefs('get');
setappdata(fig, 'pcd_html', h);
setappdata(fig, 'pcd_params', S);

h.HTMLEventReceivedFcn = @(src, evt) pcd_ui_event(fig, evt);
set(fig, 'CloseRequestFcn', @(s, ~) pcd_ui_close(s));
...
% ---------------------------------------------------------------- 关闭
function pcd_ui_close(fig)
% 关窗 = 停一切：置中止、关射频、释放 visadev（打断正在阻塞的波形读），再存参数、删窗口。
% 不能在这里 wait 采集循环 —— CloseRequestFcn 是从循环里的 drawnow 嵌进来的，一等就死锁。
% 循环用 pcd_visa('alive', token) 看到窗口没了 / abort 就会自己退。
try, pcd_visa('abort_run'); catch, end
try
    pcd_prefs('save', getappdata(fig, 'pcd_params'));
catch
end
delete(fig);
end
...
switch name
    ...
    case 'ready'    % JS setup() 完成，再推一次 MATLAB 存档，避免页面先显示 JS 缺省 2 dB
        ...
        S = getappdata(fig, 'pcd_params');
        if isstruct(S)
            pcd_ui_push(fig, struct('cmd', 'init', 'params', S));
        end
end
end
...
S = merge_params(getappdata(fig, 'pcd_params'), D);
setappdata(fig, 'pcd_params', S);
pcd_prefs('save', S);
if live
    % 调试采集正在跑时把新频率/电压立刻下发；没在跑时 pcd_debug_live 会直接返回。
    pcd_debug_live('apply', pcd_ui_html(fig));
end
end
...
allow = {};
try
    allow = fieldnames(pcd_prefs('defaults'));
catch
end
...
% ---------------------------------------------------------------- 按钮动作
function pcd_ui_action(fig, act)
if isempty(act)
    return;
end
ui = pcd_ui_html(fig);
...
switch act
    case 'stop'
        ...
        pcd_abort = true;
        pcd_visa('rf_off');
        pcd_ui_push(fig, struct('cmd', 'countdown', 'on', false));
        pcd_ui_push(fig, struct('cmd', 'status', 'text', '正在停止…'));
        return;
    case 'browse'
        act_browse(fig);
        return;
    case 'instr'
        if pcd_visa('is_busy')
            pcd_ui_push(fig, struct('cmd', 'status', 'text', '采集进行中，请先 STOP FUS。'));
        else
            pcd_instr_dialog();
        end
        return;
...
[ok, token] = pcd_visa('try_busy', fig);
if ~ok
    t = step_tab(act);
    sameFig = false;
    try
    ...
    if sameFig
        ...
        pcd_ui_push(fig, busyS);
    else
        % 别的窗口/旧循环还在：本页不要卡在「假 busy」，否则 STOP 键会变成死锁
        pcd_ui_push(fig, struct('cmd', 'busy', 'on', false));
        if ~isempty(t)
            pcd_ui_push(fig, struct('cmd', 'tab', 'tab', t));
        end
    end
    pcd_ui_push(fig, struct('cmd', 'status', 'text', '已有采集在运行。请按 STOP FUS。'));
    return;
end
...
try
    switch act
        case 'oneshot',  pcd_oneshot(ui);
        case 'debug',    pcd_debug_run(ui);
        case 'noMB'
            warn_amp(fig);
            pcd_run_experiment('before', ui);
        case 'openMB'
            warn_amp(fig);
            pcd_run_experiment('open_mb', ui);
        case 'feedback'
            warn_amp(fig);
            pcd_run_experiment('feedback', ui);
    end
catch err
    % 关窗 / STOP 引起的 visadev 失败不是用户要看的错误
    if ~is_stop_noise(err)
        pcd_ui_push(fig, struct('cmd', 'status', 'text', err.message));
        if isvalid(fig)
            errordlg(err.message, 'PCD 运行出错');
        end
    end
end
end
...
owned = isempty(token) || pcd_visa('owns', token);
pcd_visa('end_busy', token);
...
if g >= 40
    pcd_ui_push(fig, struct('cmd', 'status', 'text', sprintf('确认功放已开 ×%.4g', g)));
end
end
...
setappdata(fig, 'pcd_params', S);
pcd_prefs('save', S);
pcd_ui_push(fig, struct('cmd', 'params', 'params', S));
end
