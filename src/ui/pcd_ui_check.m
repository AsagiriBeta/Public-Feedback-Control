function ui = pcd_ui_check(ui)
%PCD_UI_CHECK 校验算法层收到的「UI 适配层」是否具备约定接口，并原样返回。
%
% 约定接口（目前唯一的实现是 pcd_ui_html：uifigure + uihtml + web/ 前端）：
...
%
% 算法层（pcd_run_experiment / pcd_oneshot / pcd_debug_run / pcd_debug_no_mb）
...
if ~isempty(bad)
    error('PCD:ui:adapter', ...
        ['界面适配层不符合约定（%s）。\n需要：%s\n' ...
         '界面实现见 pcd_ui_html，把它的返回值传给算法层即可。'], ...
        bad, strjoin(need, ' / '));
end
end
