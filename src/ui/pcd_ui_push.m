function pcd_ui_push(fig, S)
%PCD_UI_PUSH 把一条命令推给网页界面。
%
%   传输优先走 sendEventToHTMLSource（事件名 'pcd'），不要每帧写 h.Data：
...
%   图表闪烁。HTMLSource 只在 pcd_ui_app 里设一次。
%
...
p = getappdata(fig, 'pcd_push_pend');
if ~isstruct(p)
    p = struct();
end
p.(name) = val;
setappdata(fig, 'pcd_push_pend', p);
end
...
p = getappdata(fig, 'pcd_push_pend');
setappdata(fig, 'pcd_push_pend', struct());
F = struct('cmd', 'frame');
if isstruct(p)
...
last = getappdata(fig, 'pcd_push_draw_t');
nowt = tic;
if forceDraw || isempty(last) || toc(last) > 0.04
    drawnow;
    setappdata(fig, 'pcd_push_draw_t', nowt);
end
end
