function pcd_instr_dialog()
%PCD_INSTR_DIALOG 仪器设置：扫描、指派、试连接、保存到 rigol_config.ini。
%
...
% ------------------------------------------------------------ 回调
    ...
    function assign(role)
        if isempty(S.devs)
            uialert(fig, '请先点「扫描仪器」。', 'PCD', 'Icon', 'warning');
            return;
        end
        idx = selected_index(list, S.devs);
        if isempty(idx)
            uialert(fig, '请先在列表里选中一台仪器。', 'PCD', 'Icon', 'warning');
            return;
        end
    ...
    function do_test()
        ...
        if isempty(s1) && isempty(s2)
            uialert(fig, '请先扫描或填写地址。', 'PCD', 'Icon', 'warning');
            return;
        end
        if ~isempty(s1) && ~isempty(s2) && visa_same(s1, s2)
            uialert(fig, '两个地址指向同一台仪器，请分开指派后再试。', 'PCD', 'Icon', 'warning');
            return;
        end
        stat.Text = '正在试连接…';
        stat.FontColor = C.text;
    ...
    function do_save()
        ...
        s2 = edit_str(awg_e);
        if isempty(s1) || isempty(s2)
            uialert(fig, '示波器与信号源的 VISA 地址都不能为空。请先扫描。', 'PCD', 'Icon', 'warning');
            return;
        end
        if visa_same(s1, s2)
            uialert(fig, '示波器与信号源是同一台仪器（地址写法可能不同），不能保存。', 'PCD', 'Icon', 'warning');
            return;
        end
        ...
        if tx == pcd
            uialert(fig, '回读通道与 PCD 通道不能相同。', 'PCD', 'Icon', 'warning');
            return;
        end
        cfg.scope_visa = s1;
        cfg.awg_visa = s2;
