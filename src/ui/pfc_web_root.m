...
% <ctfroot>\<组件名>\ 下（组件名 = exe 名 pfc_app），web/ 实际在
% <ctfroot>\pfc_app\web。这个层级不保证跨 Compiler 版本不变，所以先试
...
cands = {fullfile(base, 'web'), fullfile(base, 'pfc_app', 'web')};
for i = 1:numel(cands)
...
% 都没找到：返回首选路径，让调用方报出完整路径（PFC:ui:noWeb）
d = cands{1};
end
