function rundir = pcd_save_acquisition(outdir, stem, S, opts)
%PCD_SAVE_ACQUISITION 一次采集写成一个目录：raw.mat + params.txt + 分析图。
%
%   rundir = pcd_save_acquisition(outdir, stem, S)
%   rundir = pcd_save_acquisition(outdir, stem, S, opts)
%
% 目录名 {stem}_{yyyymmdd_HHMMSS}/，里面是：
%   raw.mat      与旧扁平 .mat 同一套字段（顶层旧名 + params）
%   params.txt   pcd_save_params 快照的可读文本
...
if do_plots
    try
        pcd_save_run_plots(rundir, S);
    catch err
        warning('PCD:savePlots', '分析图未写出（raw.mat 已保存）：%s', err.message);
    end
end
fprintf('已保存 %s\n', rundir);
end
...
%WRITE_PARAMS_TXT 把 pcd_save_params 快照写成 key = value，方便不打开 MATLAB 就看。
fid = fopen(fp, 'w');
if fid < 0
    return;
end
cleaner = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid, '%% PCD params snapshot  (pcd_save_params)\n');
keys = fieldnames(P);
for i = 1:numel(keys)
    k = keys{i};
    fprintf(fid, '%s = %s\n', k, fmt_val(P.(k)));
