function pcd_plot_fig6_literature(rundir)
%PCD_PLOT_FIG6_LITERATURE  Fig.6 画法，频点用本实验室的，不照搬 4.5 MHz。
%
...
if nargin < 1 || strlength(string(rundir)) < 1
    error('pcd_plot_fig6_literature:rundir', '需要 run 目录');
end
rundir = char(string(rundir));
rawfile = fullfile(rundir, 'raw.mat');
if ~isfile(rawfile)
    error('pcd_plot_fig6_literature:raw', '没有 raw.mat：%s', rawfile);
end
S = load(rawfile);
...
f0_hz = f0 * 1e6;
bw = 20e3;
[~, harm_tag] = pcd_sc_harm(S);
sc_mhz = num_of(S, {'sc_band_mhz'});
if ~(isfinite(sc_mhz) && sc_mhz > 0)
    Btmp = pcd_cav_bands(f0_hz, harm_tag);
    sc_mhz = Btmp.sc_mhz;
end
np = size(pcd, 1);
sc = nan(np, 1);
...
if np >= 1 && isfinite(fs) && fs > 0
    for i = 1:np
        [F, Y] = pcd_spectrum(pcd(i, :), fs);
        [sc(i), ~, ~, ~, M] = pcd_band_energy(Y, F, f0_hz, bw, harm_tag);
        ic(i) = M.ic_ctrl;
    end
else
    sc = vec_of(S, 'RampSC');
...
metric = pcd_ctrl_metric(S, 'recorded');
end
...
sc_db = db_re(sc, sc0);
ic_db = db_re(ic, ic0);
[xx, tmode, xlab] = pcd_pulse_time_s(S, np);
xx = xx(:);
if ~(isfinite(sc_mhz) && sc_mhz > 0)
    sc_mhz = 3.0;
end
