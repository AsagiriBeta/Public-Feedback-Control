function P = pcd_save_params(kind, src)
%PCD_SAVE_PARAMS 采集存盘用的参数快照（2/3/4/oneshot/debug 同一套字段）。
%
%   P = pcd_save_params(kind, src)
%
...
if ~isfinite(P.fs_target)
    P.fs_target = 40e6;   % 与 pcd_fus_params 一致，界面没有这项
end
