function pcd_save_run_plots(rundir, S)
%PCD_SAVE_RUN_PLOTS 存盘结束时出一套分析图（每发脉冲不画，只在最终 pack_save 后一次）。
% 频谱走 pcd_spectrum / pcd_2f_quality / pcd_fft_peak_mhz，SC/IC 用已经存进
...
if n_good > 1
    ...
    try
        pcd_plot_fig6_literature(rundir);
    catch
    end
end
end
...
[xx, tmode, xlab] = pcd_pulse_time_s(S, np_axis);
xx = xx(:);
