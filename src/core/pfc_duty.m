function out = pfc_duty(cmd, val, freq_mhz, prf_hz)
%PFC_DUTY 空化率 cav_pct (%) 与周期数 n_cycle 互算（MATLAB / 网页同一套公式）。
%
% 物理定义（不要另发明）：
...
%
%   n = pfc_duty('n_cycle', cav_pct, freq_mhz, prf_hz)  % 四舍五入为整数，再截到 1–10000
%   p = pfc_duty('cav_pct', n_cycle, freq_mhz, prf_hz)
%
...
switch cmd
    ...
    otherwise
        error('pfc:duty:cmd', '未知命令 %s（用 n_cycle 或 cav_pct）', cmd);
end
end
