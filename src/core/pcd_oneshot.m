function pcd_oneshot(ui)
%PCD_ONESHOT 开环发一帧：CH1/CH2 时域+频谱上屏，写入本次目录（raw.mat + 图）。界面无关。
...
try
    ...
    if ~isfinite(realFs) || realFs <= 0
        error('PCD:oneshot:frame', '示波器未返回有效采样率，本次采集失败（未保存）。');
    end
    if isempty(chTx) || numel(chTx) < 256 || max(abs(chTx)) <= 5e-5
        error('PCD:oneshot:frame', ...
...
[F, Y, db, NFFT] = pcd_spectrum(chTx, realFs);
...
R.params = pcd_save_params('oneshot', src);
fp = pcd_save_acquisition(outdir, ['oneshot_' R.studyID], R);
