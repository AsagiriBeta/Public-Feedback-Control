...
%
% 空化率 cav_pct (%) 与周期数 n_cycle 互算见 pfc_duty：
...
% 上下限见 pfc_param_limits；网页 input / JS 截断必须抄同一套数字。
L = pfc_param_limits();
...
% 只按 pfc_param_limits 截到 1–10000，不再按假 40 MSa/s 窗静默压到 ~3000。
% 100k 点 @ 12.5 MSa/s ≈ 8 ms，1.5 MHz × 9990 周期 = 6.66 ms，盖得住。
nc = opt(S, 'n_cycle', NaN);
if ~isfinite(nc)
    nc = pfc_duty('n_cycle', opt(S, 'cav_pct', NaN), p.freq_mhz, p.prf_hz);
end
if ~isfinite(nc)
