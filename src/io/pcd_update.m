function varargout = pcd_update(cmd, varargin)
%PCD_UPDATE 检查更新 / 下载并启动新版本安装包。
%
%   info = pcd_update('check')        读更新清单，跟当前版本比
%          pcd_update('apply', info)  下载更新包并启动安装程序（装完即新版本）
%   s    = pcd_update('source')       当前生效的更新源
%   f    = pcd_update('file')         更新源配置文件路径（仅供运维覆盖）
%
% ── 更新源 ──────────────────────────────────────────────────────────
% 对用户完全透明：源写死在 default_source() 里，点「检查更新」直接用，
% 界面上没有也不需要任何配置入口：
%     https://raw.githubusercontent.com/jiaxuanli2504-cell/Passive-Cavitation-Detection/main/update.json
%
% 只有运维需要换源时（内网服务器、局域网共享、隔离网无法回源）才用得上
% <工作根>/pcd_update.ini 覆盖，界面上不暴露、出错时也不提示它：
...
% 都由 pcd_installer_asset 按版本生成，直接调 pcd_release 发版即可：
%     https://github.com/<用户>/<仓库>/releases/download/v0.2.0/PCD_Installer_v0.2.0.exe
%
...
%   {
%     "version": "0.2.0",
%     "url":     "https://github.com/me/pcd/releases/download/v0.2.0/PCD_Installer_v0.2.0.exe",
%     "notes":   "修复 XXX；新增仪器自动扫描"
%   }
%
% 更新源配置在 <工作根>/pcd_update.ini，跟 rigol_config.ini 一样是纯文本、不入库；
% 没有该文件就用 default_source() 里的默认源。发版流程见 README「软件更新」与 pcd_release。
%
...
switch lower(char(cmd))
    ...
    otherwise
        error('pcd:update:cmd', '未知命令 %s', cmd);
end
end
...
f = fullfile(pcd_root(), 'pcd_update.ini');
old = fullfile(pcd_root(), 'pfc_update.ini');
if ~isfile(f) && isfile(old)
    f = old;
end
end
...
% <工作根>/pcd_update.ini 里写 source=… 覆盖（和 rigol_config.ini 一个套路）。
s = 'https://raw.githubusercontent.com/jiaxuanli2504-cell/Passive-Cavitation-Detection/main/update.json';
end
...
info = struct('ok', false, 'available', false, ...
    'current', pcd_version(), 'latest', '', 'url', '', 'notes', '', ...
    'error', '', 'source', '');
...
info.ok = true;
info.available = pcd_version('gt', info.latest, info.current);
end
...
if ~isdeployed
    local = fullfile(pcd_root(), 'update.json');
    if isfile(local)
...
if is_url(src)
    ...
    if ~isfile(src)
        error('pcd:update:fetch', '找不到更新清单：%s', src);
    end
    txt = fileread(src);
end
...
% 供别处（如 pcd_build_exe 读上一次的清单）复用，保证只有一套解析规则
if ~isfile(f)
    error('pcd:update:manifest', '找不到更新清单：%s', f);
end
...
try
    ...
    error('pcd:update:json', '更新清单不是合法 JSON：%s', where);
end
if ~isstruct(m) || numel(m) ~= 1
    error('pcd:update:json', '更新清单格式不对（应为单个 JSON 对象）：%s', where);
end
end
...
u = sprintf('%s%s_pcd=%.3f', u, sep, posixtime(datetime('now')));
end

function s = http_error_hint(msg)
%HTTP_ERROR_HINT 把底层报错翻译成人话。
msg = regexprep(msg, '[?&]_pcd=[\d.]+', '');
if contains(msg, '404')
...
dstDir = fullfile(tempdir, 'PCD_update');
if ~isfolder(dstDir)
    mkdir(dstDir);
end
...
if ispc
    ...
    bat = fullfile(dirp, 'pcd_launch_update.cmd');
    fid = fopen(bat, 'w');
    if fid < 0
        return;
    end
