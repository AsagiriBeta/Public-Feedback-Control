function varargout = pcd_prefs(cmd, arg)
%PCD_PREFS 界面可调参数的持久化：存盘后下次启动自动恢复，不用每次重新调参。
%
%   S = pcd_prefs()              读取生效值（出厂默认 + 存档）
%   S = pcd_prefs('defaults')    只要出厂默认值（前端首次渲染用）
%       pcd_prefs('save', S)     合并写盘：空 / 非法字段不会覆盖旧值
%       pcd_prefs('reset')       删除存档，下次启动回到出厂默认
%
% 字段名清单见 default_S()，与前端 web/app.js 里的键名一一对应；
% 增删一个可调参数，只需同时改 default_S() 与 prefs_defaults()。
%
% 存档位置 <可写工作根>/pcd_prefs.mat —— 工作根由 pcd_root() 决定
...
switch lower(char(cmd))
    ...
    otherwise
        error('PCD:prefs:cmd', '未知命令 %s', cmd);
end
end
...
f = fullfile(pcd_root(), 'pcd_prefs.mat');
old = fullfile(pcd_root(), 'pfc_prefs.mat');
...
if isnumeric(S.n_cycle) && isfinite(S.n_cycle)
    pct = pcd_duty('cav_pct', S.n_cycle, S.freq_mhz, S.prf_hz);
    if isfinite(pct)
        S.cav_pct = pct;
    end
end
