function pcd_build_exe(varargin)
%PCD_BUILD_EXE 生成 Windows 独立程序（exe + 可选安装包）。
%
% 只在 Windows + MATLAB Compiler 环境下可用：MATLAB Compiler 不支持交叉编译，
% 因此 Windows 的 exe 必须在 Windows 上编译（macOS 上运行本脚本会直接报错）。
%
%   pcd_build_exe                 编译 exe，并打包含 MATLAB Runtime 的安装程序
%   pcd_build_exe('noinstaller')  只编译 exe
%
% 产物在 <项目根>/build/ 下：PCD.exe（程序本体，桌面快捷方式也叫 PCD）与
% PCD_Installer_v<版本>.exe（分发包，文件名带版本号以便区分）。目标机运行前需安装：
...
% tools/ 的上一级就是项目根。这里刻意不用 pcd_root()：本脚本可能在没跑过
% pcd_setup 的会话里被调用，直接按自身位置推算最稳。
...
if ~ispc
    error('PCD:build', ...
        ['MATLAB Compiler 不支持交叉编译：Windows 的 exe 必须在 Windows 上编译。\n' ...
         '请把本仓库拷到装了 MATLAB + MATLAB Compiler 的 Windows 机器上再运行 pcd_build_exe。']);
end
if isempty(which('compiler.build.standaloneWindowsApplication'))
    error('PCD:build', '未找到 MATLAB Compiler（需单独授权并安装），无法打包。');
end

% mcc 的依赖分析只认「编译时在路径上」的文件。src/ 或 rigol/ 不在路径上时编译不会报错，
% 但生成的 exe 运行时才报 Undefined function。这里与 pcd_app.m 的 pcd_setup 保持一致。
...
if ~isfile(fullfile(root, 'web', 'index.html'))
    error('PCD:build:web', '缺少前端资源：%s', fullfile(root, 'web', 'index.html'));
end
...
%
% 对外程序名固定为 PCD（不带版本号）：桌面图标、开始菜单、「应用和功能」都显示 PCD。
% 入口文件仍是 pcd_app.m，只改生成的 exe 名，源码不用拆一份。
iconFile = fullfile(root, 'resources', 'pcd_icon.png');
appName = 'PCD';
buildArgs = {'pcd_app.m', ...
    'AdditionalFiles', {'web'}, ...
    'OutputDir', outDir, ...
    'ExecutableName', appName, ...
    'ExecutableVersion', pcd_version()};
if isfile(iconFile)
    buildArgs = [buildArgs, {'ExecutableIcon', iconFile, ...
        'ExecutableSplashScreen', iconFile}];
else
    warning('PCD:build:icon', '未找到应用图标：%s（仍继续打包，将使用 MATLAB 默认图标）', iconFile);
end
fprintf('[1/2] 编译入口 pcd_app.m → %s.exe （版本 %s）...\n', appName, pcd_version('label'));
res = compiler.build.standaloneWindowsApplication(buildArgs{:});
fprintf('      exe 已输出到：%s\n', outDir);
...
% 会出现「装完新的，%ProgramFiles%\PCD\application\PCD.exe 还是旧的」。
% 传了之后注册表 DisplayVersion 与安装包版本都跟着 pcd_version 走。
% 发布者信息填在这里，会显示在 Windows「应用和功能」里。
publisher = 'AsagiriBeta';
appVer = pcd_version();
asset = pcd_installer_asset(appVer);   % 文件名带版本号，规则见该函数
fprintf('[2/2] 打包安装程序（RuntimeDelivery=%s，版本 %s，应用名 %s）...\n', ...
    delivery, appVer, appName);
if ~hasLocalRuntime
...
instArgs = {res, ...
    ...
    'Summary', sprintf('PCD %s', pcd_version('label')), ...
    'Description', sprintf(['PCD %s · Passive Cavitation Detection ' ...
        '(DHO814 oscilloscope / DG2052 signal generator)'], pcd_version('label')), ...
    'OutputDir', outDir};
if isfile(iconFile)
    instArgs = [instArgs, {'InstallerIcon', iconFile, ...
        'InstallerSplash', iconFile, ...
        'AddRemoveProgramsIcon', iconFile}];
end
try
    compiler.package.installer(instArgs{:});
    fprintf('      完成：%s\n', fullfile(outDir, asset.name));
catch err
    warning('PCD:build:installer', ...
        ['生成安装包失败：%s\n' ...
...
% version 直接取 pcd_version()，省掉「版本号改了但清单忘改」这个最容易漏的坑；
...
fid = fopen(f, 'w');
if fid < 0
    warning('PCD:build:manifest', '无法写入发布清单：%s', f);
    return;
end
...
% 复用 pcd_update 的清单解析（含 BOM 容错），避免两处各写一套规则。
