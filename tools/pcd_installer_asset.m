function a = pcd_installer_asset(varargin)
%PCD_INSTALLER_ASSET 安装包的文件名与下载地址 —— 全项目唯一出处。
%
%   a = pcd_installer_asset()         当前版本
%   a = pcd_installer_asset('0.2.0')  指定版本
%   s = pcd_installer_asset('slug')   从 git remote 读出的「用户/仓库」，取不到返回 ''
%
% a 的字段：
%   version  '0.1.2'
%   tag      'v0.1.2'
%   base     'PCD_Installer_v0.1.2'        给 compiler.package.installer 的 InstallerName
%   name     'PCD_Installer_v0.1.2.exe'    最终文件名
%   url      Release 下载地址（见下）
%
...
a.base = sprintf('PCD_Installer_v%s', ver);
a.name = [a.base '.exe'];
slug = repo_slug();
if isempty(slug)
